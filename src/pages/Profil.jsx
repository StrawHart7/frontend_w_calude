...
function Profil() {
  ...
  const [email, setEmail] = useState(
    localStorage.getItem("userEmail") || "ton@email.com"
  );
  ...
  const handleUpdatePassword = async () => {
    ...
    if (newPassword.length < 6)
      return setError("Mot de passe trop court (6 caractères minimum)");
    try {
      setLoading(true);
      setError("");
  ...
  return (
    <div style={{ maxWidth: "520px", margin: "0 auto", paddingBottom: "80px" }}>
        ...
        >
          <div
            ...
            style={{
              ...
              background: "#13151f", borderRadius: "20px 20px 0 0",
          ...
          >
            <div style={{
              ...
              <div onClick={() => setShowFeedback(false)} style={{ cursor: "pointer", color: "#94a3b8", padding: "4px" }}>
                <X size={20} />
              </div>
            </div>
            <iframe
            ...
            {modal === "email" && (
              <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
                <input type="email" placeholder="Nouvel email"
                  value={newEmail} onChange={(e) => setNewEmail(e.target.value)} style={modalInputStyle} />
                <input type="password" placeholder="Mot de passe actuel (confirmation)"
                  value={emailPassword} onChange={(e) => setEmailPassword(e.target.value)} style={modalInputStyle} />
            ...
            {modal === "password" && (
              <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
                <input type="password" placeholder="Mot de passe actuel"
                  value={currentPassword} onChange={(e) => setCurrentPassword(e.target.value)} style={modalInputStyle} />
                <input type="password" placeholder="Nouveau mot de passe"
                  value={newPassword} onChange={(e) => setNewPassword(e.target.value)} style={modalInputStyle} />
      ...
      <div style={{
        ...
        borderBottom: "1px solid #2d3148", overflow: "hidden",
      }}>
      ...
      {success && (
        <p style={{ background: "#1a2f1a", color: "#4ade80", padding: "12px 24px", fontSize: "14px" }}>
          {success}
        </p>
      )}
      ...
      <div style={{ background: "#13151f", borderRadius: "12px", margin: "0 16px", overflow: "hidden" }}>
        <SettingItem icon={<User size={18} color="#fff" />} iconBg="#6c63ff"
          label="Modifier l'email" onClick={() => { setError(""); setModal("email"); }} />
        <SettingItem icon={<Lock size={18} color="#fff" />} iconBg="#f59e0b"
      ...
      <div style={{ background: "#13151f", borderRadius: "12px", margin: "0 16px", overflow: "hidden" }}>
        <SettingItem icon={<Star size={18} color="#fff" />} iconBg="#f59e0b"
          label="Noter l'app" onClick={() => navigate("/coming-soon")} />
        <SettingItem icon={<MessageSquare size={18} color="#fff" />} iconBg="#3b82f6"
      ...
      <div style={{ margin: "24px 16px 8px", background: "#13151f", borderRadius: "12px", overflow: "hidden" }}>
        <SettingItem icon={<LogOut size={18} color="#f87171" />} iconBg="#2d1f1f"
